"""
Using Google Vision API to Extract Caption from Images and Translate it with Google Translate API
Imagine that we have a scenario where we need to get written information from an image and then translated it to desired locales as what's written inside of this https://www.cloudskillsboost.google/focuses/12704?parent=catalog
"""
# How to use: python ml_vision_api_integration.py --bigquery-dataset image_classification_dataset --bigquery-table-name image_text_detail --project-name $DEVSHELL_PROJECT_ID --bucket-name $DEVSHELL_PROJECT_ID --desired-locale ja
import os
import sys
import logging
import argparse
...
def get_args():
    ...
    parser.add_argument("--bigquery-table-name")
    parser.add_argument("--project-name")
    parser.add_argument("--bucket-name")
    parser.add_argument("--desired-locale")
...
desired_locale = args.desired_locale
...
logging.info('Processing image files from GCS. This will take a few minutes..')
...
for file in files:    
    if file.name.endswith('jpg') or  file.name.endswith('png'):
        ...
        if locale == desired_locale:
            ...
            # For non EN locales pass the description data to the translation API
            ...
            translation = translate_client.translate(text_data, target_language=desired_locale)
...
logging.info('Writing Vision API image data to BigQuery...')
