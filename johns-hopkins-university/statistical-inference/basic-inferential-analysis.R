...
ggplot(ToothGrowth, aes(x = len)) +
  geom_histogram(bins = 6,
                 ...
                 colour = "black") +
  geom_vline(
    xintercept = mean(ToothGrowth$len),
    lwd = 1,
    colour = "red"
  ) +
  geom_vline(
    xintercept = median(ToothGrowth$len),
    lwd = 1,
    colour = "green"
  ) +
  annotate(
...
a <-
  ToothGrowth %>% group_by(supp) %>% summarise(meanlen = mean(len)) %>%
  ggplot(aes(x = supp, y = meanlen, fill = supp)) +
  geom_bar(stat = "identity") +
  geom_text(
    aes(label = round(meanlen, digits = 1)),
    vjust = 4,
    colour = "white",
    size = 5
  ) +
...
c <-
  ToothGrowth %>% group_by(dose) %>% summarise(meanlen = mean(len)) %>%
...
b <-
  ToothGrowth %>% group_by(supp, dose) %>% summarise(meanlen = mean(len)) %>%
...
freedom <- function(s_x = 1, s_y = 1, n_x, n_y) {
  p <-
    ((s_x ^ 2 / n_x) + (s_y ^ 2 / n_y)) ^ 2 / (((s_x ^ 2 / n_x) ^ 2 / (n_x -
                                                                         1)) + ((s_y ^ 2 / n_y) ^ 2 / (n_y - 1)))
  print(p)
}
...
spool0.5_1.0 <- sqrt((std0.5 ^ 2 * 20 - 1 + std1.0 ^ 2 * 20 - 1) / (20 +
                                                                      20 - 2))
delta0.5_1.0 <-
  mean(ToothGrowth$len[ToothGrowth$dose == 0.5]) - mean(ToothGrowth$len[ToothGrowth$dose == 1])
p1 <- power.t.test(
  n = 20,
  delta = delta0.5_1.0,
  sd = spool0.5_1.0,
...
spool1.0_2.0 <- sqrt((std1.0 ^ 2 * 20 - 1 + std2.0 ^ 2 * 20 - 1) / (20 +
                                                                      20 - 2))
delta1.0_2.0 <-
  mean(ToothGrowth$len[ToothGrowth$dose == 1]) - mean(ToothGrowth$len[ToothGrowth$dose == 2])
p2 <- power.t.test(
  n = 20,
  delta = delta1.0_2.0,
  sd = spool1.0_2.0,
